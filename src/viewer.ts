...
class Viewer {
    ...
    //WDD
    private operationTimer = 0;
    private readonly operationInterval = 1.0 / 30.0; // 单位：秒）
    ...
    update(deltaTime: number) {
        ...
        if (this.entities.length > 0 && this.operationTimer >= this.operationInterval) {
            this.operationTimer = 0;

            for (let i = 0; i < this.entities.length; i++) {
                this.entities[i].enabled = (i === this.currentEntityIndex);
                const sorter = this.entities[i]?.gsplat?.instance?.sorter;
                sorter.enabled = false;
                if(i === this.currentEntityIndex)
                {
                    // 手动执行主线程排序
                    const pos = this.camera.getPosition().clone();
                    const dir = this.camera.forward.clone(); // 👈 反转 forward 得到观察方向
                    
                    pos.x = -pos.x;
                    pos.y = -pos.y;
                    dir.z=  -dir.z;

                    sorter.forceSortSync(pos, dir);
                } 
            }
